...
// Shape of each item in poSummary returned by getDepartmentSummary static method:
// { poNumber: "PO1", averageAttainment: 2.56, courseCount: 1, courses: [...] }
...
export default function AdminDashboard() {
  ...
  // Backend returns (via ApiResponse.success spread):
  // data: { department, academicYear, courses: [...], attainments: [...], poSummary: [...] }
  // "attainments" = one record per course that has had attainment calculated
  const attainments = departmentSummary?.data?.attainments || []
  const coursesWithAttainment = attainments.length

  // poSummary shape: [{ poNumber: "PO1", averageAttainment: 2.56, courseCount: 1, courses: [...] }]
  // DepartmentSummaryChart expects: Record<string, number | null> → { "PO1": 2.56, "PO2": 2.6, ... }
        ...
        <Card>
          ...
          <CardContent>
            {summaryLoading ? (
              ...
              <div className="text-2xl font-bold">{coursesWithAttainment}</div>
            )}
          </CardContent>
        </Card>
      </div>
